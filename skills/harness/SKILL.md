...
<HARD-GATE>
When implementor:harness is active, it is the sole orchestrator. Do NOT invoke implementor:run, implementor:auto-plan, or any other implementor pipeline skill. The harness generates infrastructure — it does not execute development tasks. If the user's request is "build X" (a feature), route them to implementor:run. If their request is "build a harness for this project" or "design an agent team," this is the right skill.
...
This project's agent team was generated by `implementor:harness`.
...
- To update the harness: `/implementor:harness`
...
This skill is standalone but complements the implementor pipeline:

- **implementor:harness** — generates the agent team infrastructure (this skill)
- **implementor:run** — executes development tasks (can use harness-generated agents)
- **implementor:auto-verify** — can verify harness-generated systems against real running instances

The harness generates infrastructure. The implementor executes within that infrastructure.
