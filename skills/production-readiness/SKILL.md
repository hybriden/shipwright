...
8. **Code hygiene** — new code has no TODO/FIXME/HACK, no commented-out blocks, no debug logging (console.log/print/debugger), no unused imports/vars, no comment noise (every comment passes the delete test in `../_shared/comments.md` — states a why/constraint/footgun the code cannot); clear names; consistent formatting. (`grep -rn "TODO\|FIXME\|HACK\|console\.log\|debugger" [changed files]`)
