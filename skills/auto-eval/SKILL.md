...
| 4 | Efficiency | run report's Pipeline Results metrics (wall-clock per phase, suite runs, ledger reuses, subagent dispatches), auto-debug count, review rounds, net-positive rollbacks | churn within the band for the task size; no unchanged commit re-tested |
