...
Read the architecture map first (per `../_shared/architecture-map.md`) to identify involved modules, the blast-radius dependency chain, hot spots, and interface contracts; record this as the runbook's "Architecture Context" (fix subagents consume it). Then gather (in order) from: the user's message; project docs (docs/, README, CLAUDE.md); `.shipwright.json` (`startCommand`, `verifyCommand`, `verifyUrl`); previous runbooks (`docs/*RUNBOOK*`); project tools (tools/, scripts/, Makefile, package.json).
...
3. **Execute verification** by system type. Web: Playwright (navigate → snapshot verify elements+values → interact → snapshot verify state → screenshot → console_messages zero errors → network_requests all 2xx); multi-page journeys prove state persists across navigation. API: curl, assert status + body values (jq), pagination, error messages, auth. CLI/pipelines: run, assert exit code, validate output with the project's own tools, inspect output contents. CMS / file-format / DB / multi-service specifics: `references/verification-strategies.md`.
