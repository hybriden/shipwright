...
- `skills/_shared/` - Shared reference snippets linked by multiple skills (architecture-map consumption, net-positive gate, evidence evaluation, runtime probing by app type, project-tool discovery, context budget, checkpoints, iteration loop contract, minimalism ladder, SOLID / DRY / KISS principles, dotnet-skills consumption protocol, react-doctor verify gate) to avoid duplication
